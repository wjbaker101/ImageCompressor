...
namespace ImageCompressor.Main
{
    public class Compressor
    {
        ...
        public async Task<byte[]> GetCompressedImageFromFileAsync(string fileLocation)
        {
            var source = File.ReadAllBytes(fileLocation);
            return await Tinify.FromBuffer(source).ToBuffer();
        }

        public async Task CompressImage(string fileLocation, string destinationFile)
        {
        ...
        public async Task ResizeImage(string fileLocation, string destinationFile, dynamic options)
        {
            var source = Tinify.FromFile(fileLocation);
